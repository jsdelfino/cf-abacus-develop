...
const aggregate = function *(aggrs, u) {
  ...
  // Omit the incoming doc revisions unless we're sampling down the
  // reduction result docs
  const a = sampling === 1 && aggrs[0] ? omit(aggrs[0], '_rev') : aggrs[0];
  const c = sampling === 1 && aggrs[1] ? omit(aggrs[1], '_rev') : aggrs[1];
  ...
  */
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    newa.space(u.space_id).consumer(u.consumer_id || 'UNKNOWN',
      u.space_id, u.processed,
      dbclient.kturi(okeys(u)[1], seqid.sample(dbclient.t(u.id), sampling)));

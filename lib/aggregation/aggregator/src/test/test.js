    ...
    const get = (done) => {
      ...
      transform.map(usage, (u, i, l, cb) =>
        request.get(locations[u.id], {}, (err, val) => {
          ...
          // Expect one of the test aggregated values
          map(aggregated, (a, i, l) => {
            try {
              expect(omit(val.body, 'id', 'accumulated_usage_id'))
                .to.deep.equal(omit(a, 'accumulated_usage_id'));
              checks[i] = true;
            }
            catch(e) {
              return false;
          ...
          // Expect to have seen our final test aggregated values
          expect(checks[3]).to.equal(true);
          done();
        });
    };

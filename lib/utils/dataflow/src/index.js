...
const mapper = (mapfn, opt) => {
  ...
  const play = function *(req, idoc, auth) {
  ...
  routes.post(opt.input.post, throttle(function *(req) {
    ...
    return yield play(req, req.body, auth);
  }));
...
const replay = (routes, twindow, auth, cb) => {
  // Use the configured replay time, default to 6 hours
  const tw = process.env.REPLAY ? parseInt(process.env.REPLAY) :
    twindow ? twindow : 1000 * 3600 * 6;
  debug('Replaying last input docs since %d msec back', tw);
  ...
  yieldable.functioncb(function *() {
    ...
    const rlogs = treduce(ilogs, function *(rlogs, ilog, i, l) {
      ...
      const rlog = yield routes.play(undefined, ilog, auth);

...
// Replay last input docs as needed for recovery
const replay = function *(routes, twindow) {
  // Use the configured replay time
  const tw = process.env.REPLAY ? parsInt(process.env.REPLAY) : time;

  debug('Replaying last processed inputs, back %d msec', tw);
  return true;
};

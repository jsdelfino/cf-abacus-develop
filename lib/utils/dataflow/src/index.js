...
const lastAccum = function *(idoc, okey, otime, ocache, odb) {
  ...
  odebug('Retrieved accumulated output doc id %s, rev %s, %o',
    odocs.rows[0].doc.id, odocs.rows[0].doc._rev);
  debug('Retrieved accumulated output doc id %s, rev %s, %o',
    odocs.rows[0].doc.id, odocs.rows[0].doc._rev, odocs.rows[0].doc);
  // return dbclient.undbify(odocs.rows[0].doc)
  return odocs.rows[0].doc;
};
...
const logOutput = function *(odoc, odb) {
  odebug('Logging output doc %s, rev %s', odoc.id, odoc._rev);
  debug('Logging output doc %s, rev %s', odoc.id, odoc._rev);
  try {
    const res = yield odb.put(odoc);
    odoc._rev = res.rev;
  }
  catch(err) {
    odebug('Error logging output doc %s, rev %s, %o',
      odoc.id, odoc._rev, err);
    debug('Error logging output doc %s, rev %s, %o',
      odoc.id, odoc._rev, err);
    throw err;
  }
  odebug('Logged new output doc %s, rev %s', odoc.id, odoc._rev);
  debug('Logged output doc %s, rev %s, %o', odoc.id, odoc._rev, odoc);
};

// Log a list of output docs
const logOutputs = function *(odocs, odb) {
  const ids =
    map(odocs.concat([]).reverse(), (doc) => pick(doc, 'id', '_rev'));
    ...
    try {
      ...
      if(odb && !errors.length) {
        ...
        const allpodocs = reduce(pgdocs, (allpodocs, podocs) => {
          return allpodocs.concat(podocs);
        }, []);
        yield logOutputs(allpodocs, odb);

        // Cache all the accumulated output docs
        map([last(calls)], (call, i, l) => {
          cacheAccums(
            itype, call[0].idoc, last(pgdocs), call[0].okeys, ocache, odb);
        });
      }
